...
- **humanizer-zh**:文本去 AI 味、语言自然化、人物语气与节奏优化(在本项目内统一并入为 `humanize` 能力;对外中文别名统一叫 **去AI味**)
...
**工作模式规则:**
- 所有**配置和调试指令**(查看 log、doctor、模型配置、naming lint、smoke 等)**永远走系统模型**,不受工作模式影响
- 只有**创作、写作、修稿**等产出内容的任务才受工作模式管辖:
...
Novel Studio 会为小说创作、润色、重写、修改等**产生新内容**的任务保存独立历史文件。

**历史目录**:`<project>/.novel-studio/history/`

**每条历史是一个独立 Markdown 文件**，文件名形如 `YYYY-MM-DD_HHmmss_<chapter>_<type>_<id>.md`，包含：
- 创作目的 / 用户要求
- 任务类型(write_chapter / rewrite / humanize / review 等)
- 工作模式(system / direct)
- 生成模型信息(provider、model、API、baseUrl、temperature、maxTokens 等)
- 输入/输出字符数、耗时等统计
- 生成结果正文

**管理脚本**:`python3 scripts/ns_history.py <cmd> ...`

常用命令:
```bash
# 记录一条历史(系统模型生成内容后手动调用)
python3 scripts/ns_history.py record <project> <chapter> \
  --type write_chapter \
  --purpose "<创作目的/要求>" \
  --result-file <生成结果文件> \
  --model "<当前模型>" \
...
# 查看历史列表
python3 scripts/ns_history.py list <project> [--chapter ch_001] [--recent 10]

# 查看单条历史详情
python3 scripts/ns_history.py view <project> <history-id>
```

**自动记录规则**:
- `scripts/direct_api_writer.py --execute` 成功生成内容后,会自动写入 history。
- 系统模型模式下,只要 NS 系统模型产出了新的正文/润色/修改结果,应在任务完成前调用 `ns_history.py record` 写入 history。
...
7. **润色模式**:文笔优化、去 AI 痕迹、人物语气差异化(内部能力名:`humanize`;对外中文叫法:**去AI味**)
...
15. **直连 API 写作模式**:让 Novel Studio 自己读取 input pack 并调用指定 OpenAI-compatible 模型接口,隔离当前聊天上下文,默认 dry-run 不真实请求

**默认规则:**
- 不跨模式乱跳。
- **NS 模型隔离硬规则**:
  - **配置/调试指令**(模型配置、查看 log、doctor、naming lint、smoke、consistency audit 等)**永远走 OpenClaw 系统对话模型**,不走直连 API。
  - **创作/写作/修稿类任务**(写作、续写、重写、审校、润色/去AI味、风格提炼、学习诊断、镜头跟随判断、第三方描述判断、场景张力判断、信息释放判断等)**受全局工作模式管辖**:
    - 当前为「系统模型」模式 → 走 OpenClaw 系统对话模型
    - 当前为「直连模式」 → 先确认 `.novel-studio/global-config.json` 存在 `directApi` 配置,通过 `scripts/direct_api_writer.py` 执行
  - 切换工作模式:`python3 scripts/ns_model_config.py workmode set system|direct`;查看当前模式:`python3 scripts/ns_model_config.py workmode show`
...
- 若直连模式下缺少 direct API 配置,先提示并执行/建议:`python3 scripts/ns_model_config.py global set`;配置完成前停止直连 API 内容生成类 NS 任务。
- 若缺少必要输入,先补关键约束,再继续。
...
- 若用户要"NS 直连 API / 指定模型写作 / 不用系统模型 / 隔离聊天上下文 / direct api writing",进入**直连 API 写作模式**。
...
如需先做保守型去 AI 味处理,优先使用:
- `scripts/workflow_runner.py <root> <chapter> humanize [light|medium|heavy]`
- 默认档位:`medium`
- 输出会写入:`.novel-studio/logs/<chapter>-humanize-pass.md`
- 当前策略:**只生成旁路稿,不直接覆盖原章**
...
| 用户说 | 执行 |
|--------|------|
| **配置模型** / 配置 NS 模型 / 选模型 | 强制进入模型配置模式,走系统模型列出可用模型并交互选择 |
| **系统模型** / 切系统模型 / 用系统模型 | 切换工作模式为 `system`,所有任务走 OpenClaw 对话 |
| **直连模式** / 切直连 / API 模式 / 用直连 API | 切换工作模式为 `direct`,创作任务走 direct API |
...
- "NS 直连 API / 指定模型写作 / 不用系统模型 / 隔离聊天上下文 / direct api writing" → 直连 API 写作模式
- "把这段改得自然点" → 润色模式
...
- **NS 内容任务受工作模式管辖**:
  - 配置/调试指令永远走系统模型。
  - 系统模型模式下,所有任务走当前系统对话模型。
  - 直连模式下,创作/写作/修稿任务必须走 `direct_api_writer.py`;若缺少 `.novel-studio/global-config.json` 的 `directApi` 配置,先要求配置模型(`python3 scripts/ns_model_config.py global set`)。
- **不要擅自推进到下一章**,除非用户明确要求
